...
var Caching = require('caching'),
    responseCache = new Caching('redis');
...
function _createJourneyHandler(module, method, options){
  var handler = function (res, resource, id, params) {
    ...
    if(module[method].cache || (module.cache && module[method].cache !== false)){
      var cacheIdentifier = [method, JSON.stringify(params), res.request.url.query].filter(function(v){ return !!v; }).join("-"),
          ttl = isNumber(module[method].cache) ? module[method].cache : (isNumber(module.cache) ? module.cache : 3600000);
      
      responseCache(cacheIdentifier, ttl, function(passalong) {
        var _send = res.send;
        res.send = function(statusCode, headers, body){
          res.send = _send;
          passalong(null, { statusCode: statusCode, headers: headers, body: body });
        };
      
        finalize();
      
      }, function(err, c) {
        res.send(c.statusCode, c.headers, c.body);
      });
    } else {
      finalize();
    }
    ...
    function finalize(){
      ...
      try{
        ...
        if(typeof module[method].schema == 'undefined'){
          ...
          module[method].apply(self, self.request.params);
          return;
        }
  ...
  return function(err, result, response){
    try{
      ...
      if(err){
		...
		if(typeof result === 'object' && result.contentType && result.bodyBuffer){
